...
module.exports = function oniyiConfig(options) {
  ...
  const env = toLowerCase(options.environment || options.env || process.env.NODE_ENV || 'development');
  ...
  if (!directories.length) {
    const basePath = options.sourceDir || options.baseDir || options.basePath || path.join(process.cwd(), 'config');
    logger.debug('directories array is empty, adding computed basePath "%s"', basePath);
