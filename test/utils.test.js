...
test('makeFilenameRegex() returns regular expression that tests truthy for all file name patterns', (t) => {
  ...
  t.true(fileNames.every(item => rex.test(item)), `RegEx should test truthy for ${fileNames}`);
});
...
test('safeRequire() returns `undefined` if module does not exist', (t) => {
  const mod = safeRequire(path.resolve(__dirname, './foo'));
  t.true(mod === undefined);
});

test('safeRequire() returns requested module if exist', (t) => {
  const mod = safeRequire(path.resolve(__dirname, './fixtures/config-files/providers.js'));
  t.true(!!mod);
});
...
test('mergeObjects() two arrays', (t) => {
  ...
  const isUnionedArray = Array.isArray(merged) &&
    merged.every((val, idx) => val === unioned[idx]);
